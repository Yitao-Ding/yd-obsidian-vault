...
## 平成たち祭 vol.2 定点 (2026-09-15)

- 素材: `~/Downloads/平成たち祭定点/` FX30 定点 16本 (A026C020〜C044)、うち C031 は2秒の誤録画。15本 = 香盤表の15ナンバー、撮影順 = 香盤順だった
- 投影はロゴのみで映像から名前は読めない。**影アナが冒頭でチーム名を読み上げる**ので、冒頭75秒を 16k mono wav に抜いて openai-whisper medium (CPU、16本で約7分) で文字起こし → 香盤表 PDF (fitz で text 抽出) と照合。固有名詞の表記は崩れる (薬=KUSURI、ハリネトユー=Haruto Crew、スパーキングメテオ=秘密結社火花起隕石) が、人数・順番と合わせれば全件同定できた
- 1ファイル=1リンク方式: `_tools/run_tachi.py` (gfup.py の upload_verified を直接呼ぶ、matomete 不使用、order.txt 順、tachi_results.jsonl に追記・再実行可)。70GB/15本が 19分 (約62MB/s、44.gigafile.nu)
- 一覧: `~/Downloads/平成たち祭定点/納品リンク一覧.txt`。期限 2026-12-24
