...
## 2026-09-15 「ジブリのような街」をマニラの観光地として登録した #hallucination
- 何をした: 8/7 の Gemini 議事録の要約に「マニラ観光と世界遺産の訪問計画」として「ジブリのような街」と「イントラムロス」が並んでいたのを、そのままマニラの候補として Notion に入れた
- 何が問題: 文字起こしの本文を読むと、ジブリのような街はコニーが案内したセブの候補地域 (辻の呼び方で「バランガタポ」)。Gemini が菅野の「行きたい場所」2つを1段落にまとめただけで、地理的な関係は無かった。YD から「どういう意味?」と聞かれて発覚
- 再発防止: Gemini の要約段落は「同じ話題で語られた」ことしか保証しない。場所や団体を登録するときは、要約ではなく文字起こし本文の該当箇所を grep して、誰が・どこの話として言ったかを確認してから書く。CFA の件と同根で、今日2回目
