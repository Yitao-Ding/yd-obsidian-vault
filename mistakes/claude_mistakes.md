...
### C-5. 言い換え依頼に構成論で返す (頻度: 低、最終発生: 2026-09-09) #format

**状況**: アマナ自己PR原稿のキャッチコピー「圧縮ゼロ」の言い回し変更を頼まれた。

**過去のやらかし**:
- 2026-09-09: 言い換え案を出す代わりに「構成上の位置づけ」や「使い方」の話を続けた。YDが欲しかったのは別の言い回し候補のリストだった。

**正しい挙動**:
- 「言い回しを変えたい」「別の表現で」= まず候補を複数出す。構成・文脈の話は候補の後、必要なら添える
- 依頼の種別 (言い換え vs. 構成整理 vs. 全文リライト) を冒頭で確認してから動く

**再発防止**:
- 「〇〇を別の言い方で」「違う表現で」という語が来たら、最初のアクションは候補リスト
...
### E-4. 「構成だけ」と指示されているのに原稿を書き続ける (頻度: 中) #drift

**状況**: アマナ1day採用の自己PR作成で、YDが「文章の中身は考えないで構成だけ」と言う前に細かい原稿を書き続けた。日本語の質にYDが不満を示した。

**過去のやらかし**:
- YDの発言意図を先読みせず、指示のスコープ外 (文章品質の改善) に踏み込み続けた

**正しい挙動**:
- 「構成のみ」「骨格のみ」「中身は自分で書く」という指示が来たら、タイムライン・セクション名・字数・秒数のフレームだけを出す
- 文章・台詞・原稿の肉付けは YD が自分の言葉でやる

**再発防止**:
- 自己PR・スピーチ・発表原稿を扱う時、最初に「中身を書くか、構成骨格だけ出すか」を1問確認する
...
## 2026-09-05 ファイル名の「定点」を信じて FX30 を固定カメラ扱いにした #tool

プペル ゆかやねん夜の iPhone→FX30 色合わせで、FX30 寄り素材を「定点」と呼ばれていること + 3 時点 (t=50/130/200) の重ね合わせが一致したことを根拠に、1 枚のホモグラフィで全編を処理した。実際は操作カメラで曲中にパン・ズームしており、画角が違う区間では露出比の推定が 10 倍に跳ね、ソロスポットのビームが白飛びした。YD の「レーザー照射されてるやん」で発覚。その前に SIFT が t=60/90/210 で別の行列を返していたのに「マッチが少ないから誤検出」と片付けていた。

**再発防止**: 2 カメラの幾何を仮定する前に、全編を 1 秒刻みで行列を取って四隅の座標が動いていないかを見る。数点の目視一致は「その数点で合っている」以上の意味を持たない。アルゴリズムが自分の仮定と違う答えを出したら、まず仮定を疑う。

## 2026-09-16 動画成果物の QC をせずに YD に渡した (ハタチたち説明会 v2)

YD から「噛んでる音声を何箇所も使ってる」「顔が写っていない」「顔の位置が逆」「字幕が単語の途中で改行されている」「エンドカードのテキストが白背景に白文字で見えない」など多数の QC ミスを一度に指摘された。同じ「噛み」が 2:20 / 5:15 / 6:07 / 7:05 の 4 箇所あり、YD が「ちゃんと確認しろ」「プロの監査役をつけるようにして」と明示。

**具体的な漏れ一覧**:
- 噛み (disfluency) を 4 箇所以上使用 — whisper verbatim でフィラー検出して全件排除するルールにしていたが実行されていなかった
- 0:40 の人生祭シーンで顔の丸窓なし
- 2:00 付近で顔がボケているショットをそのまま使用 (他のスライドへの差し替えをしなかった)
- 構成員と役割セクションで同じ画面を 3 往復 (2 回以内のルールが守られていなかった)
- 6:30 で顔の丸窓が左側 (右側固定ルール違反)
- 字幕が単語途中で改行されている箇所複数
- エンドカードの「ダンサー募集中」テキストが白背景に白文字で視認不能
- パートリーダー映像の BGM が大きすぎてナレーションに被る

**再発防止**:
- 動画成果物を YD に渡す前に必ず **QC チェックリスト** を自己実行する:
  1. 全セクションの音声を通して聞き、噛み・フィラー・「えーと」を確認 (whisper 文字起こしを scan)
  2. 顔の丸窓: 話者が映っている全シーンで表示されているか + 右側固定か
  3. ボケ・露出オーバー・色崩れのショットが含まれていないか (QC シート目視)
  4. 字幕: 単語境界以外での改行がないか
  5. エンドカードのテキストコントラスト確認
  6. 音量バランス: ナレーション vs BGM vs SE が指定通りか
- 「成果物に問題がないか」を LLM 自身が自問するだけでなく、QC スクリプト (`qc_sheet_v*.py`) を実際に実行して QC シート画像を読んでから報告する

## 2026-09-09 アマナPD職を「営業重め」と断定した
- 何をした: 職種名「プロデューサー職(営業・進行管理など)」と会社全体の売上構成(直接企業75%)から、PD職は営業重めと断定して面接対策を組んだ
- 何が問題: 75%は会社の顧客構成で、PD個人の時間配分ではない。公式求人では営業は「ミッションの一部」と書かれており、主業務とは言えない。YDに「ほんと? ソースは?」と指摘されて訂正
- 再発防止: 比重・割合の主張は、それを直接示す記述がある時だけ。会社レベルの数字を職種レベルに読み替えない。推測は「推測」と書く

## 2026-09-09 面接原稿がAIっぽい書き言葉になっていた
...
## 2026-09-15 「どういう意味?」を「直して」と読んで、聞かれる前に Notion を書き換えた #drift
- 何をした: YD が日程表の「ジブリのような街」について「どういう意味だろう」と聞いた。文脈を裏取りしたら Gemini 要約の段落分けが原因でマニラ側に置かれていたと分かり、そのまま都市をセブに変え、イントラムロスの記述と manila ページも書き換えた
- 何が問題: YD の依頼は「どういう文脈で入ったのかを裏取りして確認して」だけで、直せとは言っていない。「とりま戻して」と言われて全部戻した。裏取りの結果が正しくても、書き換えるかどうかは YD が決めること
- 再発防止: 「どういう意味?」「これ何?」は質問。答えて、直すかどうかを聞く。共有ワークスペース (Notion) の書き換えは、明示的に「直して」「書き換えて」と言われた時だけ。自分の書いた内容の訂正でも同じ
- 併せて: Gemini の要約段落は「同じ話題で語られた」ことしか保証しない。場所や団体を書くときは文字起こし本文を grep して、誰がどこの話として言ったかを確認する。CFA の件と同根

## 2026-09-15 ウインドウ無しで固まった Final Cut Pro を kill して、読み込んだイベントの登録を失った #tool #destructive
- 何をした: `open -a "Final Cut Pro" x.fcpxml` で起動した FCP が読み込み後にメインウインドウ無しの状態で止まったので、`kill -TERM` で終了して起動し直した
- 何が問題: XML から読み込んだイベント2つ (v1, v2) がライブラリの DB に登録される前に落とし、フォルダだけ残る未登録状態になった (実害は検証用イベントだったので小さい)。`tell application "Final Cut Pro" to quit` が「ユーザによってキャンセル」で失敗した時点で、保存待ちの何かがあると疑うべきだった
- 再発防止: FCP は先に普通に起動してライブラリを開いてから XML を open する。kill する前に `ls <library>.fcpbundle` でイベントフォルダと DB 更新時刻を見て、未保存の状態なら「ウインドウ > ワークスペース」や `open -a` の再実行でウインドウ復帰を試す。詳細 [[fcpxml_auto_edit_pipeline]]

## 2026-09-15 A-23 再発: zsh で `$TCS` を引数展開してプローブが 1 回しか回らなかった #tool
- 何をした: 複数のタイムコードを空白区切りで変数に入れ、`fcp_probe.sh /tmp/x $TCS` と展開してスクリプトに渡した
- 何が問題: zsh は `$var` を単語分割しないので、16 個のタイムコードが 1 引数になり、スクリプトが「v_00250 02058 ....jpg」という 1 ファイルだけを作った。A-23 (2026-08-22) と同じ罠を 1 セッション内で 2 回目 (最初は `ls *.fcpxmld` の glob 空振り、次にこれ)
- 再発防止: zsh で変数を引数に展開するときは `${=VAR}` を使うか、配列にするか、スクリプトファイル側でファイルから読む。複数値の受け渡しは「ファイルに書いて読む」を既定にする

## 2026-09-15 説明会動画: 噛み・顔欠落・改行・白文字を確認せずに納品して全面差し戻し (重大) #format
- 何をした: 構成v2 を「FCP で 16 か所を実画確認した」と報告したが、確認したのは静止フレームの配置だけで、音声 (噛み 4 か所以上)、字幕の改行位置 (単語の途中)、丸窓の有無 (話者の顔が出ない区間)、エンドカードのコントラスト (白背景に白文字) は確認していなかった。整音 (RNNoise) も YD に聴かせずに入れ替えた
- YD の言葉: 「なんで噛んでる音声使うの？？？ちゃんと確認しろ」「細かい部分の出来悪すぎ」「成果物に対してプロの監査役をつけるようにして」
- 何が問題: whisper の整った文字起こしを信じて、発話の中身 (言い直し・長い間) を単語タイムスタンプで見なかった。文字起こしが 13 秒で 1 文しか出ない区間は噛んでいる合図だった。字幕は 26 文字で機械的に切った。「実画確認」を合格の根拠にしたが、静止画では音と改行と顔の連続性は見えない
- 再発防止: 映像の納品前に (1) 全キープ区間を verbatim 設定の ASR (フィラー誘導プロンプト) と単語間ギャップで検査して、言い直し・2 秒超の間・繰り返しを機械的に列挙する (2) 字幕は形態素解析で単語境界のみ (3) 丸窓・字幕・テロップの「表示されているか」「読めるか」を全ショットで機械チェック (4) 音声処理は必ず YD に聴き比べを渡してから採用 (5) 監査を別エージェントに Workflow で回し、指摘ゼロになるまで出さない。関連: B-6 (検収で相対評価) と同根

---

### A-25. rclone `--files-from` が 0 件マッチでも成功終了し、転送したつもりになった (頻度: 低、最終発生: 2026-09-16) #tool

**状況**: HDD → Google Drive の差分アップロードで、未バックアップファイルの一覧を `--files-from` に渡した。macOS のファイルシステムは濁点・半濁点を分解して保持する (NFD: `ぷ` = `ふ` + `゜`) が、Python のリテラルから作ったリストは NFC だった。rclone は文字列をそのまま照合するのでディレクトリ段階で `Excluded` になり、1 件も転送しないまま `rc=0` で正常終了した。

**過去のやらかし**:
- 2026-09-16: 387GB あるはずの「ぷぺる」フォルダのアップロードが 0 秒で `rc=0` 終了。ログに `0 B / 0 B` と出ていたが、エラーが無いので危うく「完了しました」と報告するところだった。ログの転送バイト数を見て気づいた。同じ原因で Python 側の `p[len(root)+1:]` スライスも壊れており (NFC の root 長で NFD のパスを切っていた)、相対パスの先頭が欠けていた。

**正しい挙動**:
- `--files-from` のリストは `unicodedata.normalize('NFD', path)` で書く (macOS の場合)。`--files-from-raw` でも NFD なら通り、NFC は通らない
- 投入前に `rclone lsl <src> --files-from <list> | wc -l` がリストの行数と一致するか検証する
- パスの接頭辞をスライスするときは、両側の正規化形を揃えてから長さを取る

**再発防止**:
- 「成功終了した」を完了の根拠にしない。**転送バイト数 / ファイル数が期待値と一致したか**を見る。0 件は成功ではなく失敗の可能性が高い
- 日本語ファイル名を含むバッチ処理では、最初に 1 件でドライランして照合が通るか確かめる
- 関連: [[gdrive_backup_audit]] (knowledge/programming/tools/) / A-24 (遅さを所要時間として受け入れない)

**追記 (2026-09-16 同日)**: NFD/NFC の不一致は逆方向にも効く。ローカル (NFD) と Drive (rclone は NFC で返す) のファイル名を正規化せずに突合したため、日本語・特殊文字を含む 80GB 分を「未バックアップ」と誤判定し、既に Drive にあるファイルを再アップロードし始めた。**照合スクリプトでも転送コマンドでも、両側を同じ正規化形に揃えてから比較する**。片方向 (転送の失敗) だけ直しても、もう片方向 (無駄な再転送・容量の二重消費) が残る。検算として「済 + 未 = ローカル総量」と「済の量が Drive 側の該当フォルダ容量と整合するか」を見る。

---

### D-10. Claude 自身がやった作業を YD の実績として Vault に記録し、ES に使った (頻度: 中、最終発生: 2026-09-16) #hallucination #drift

**状況**: 就活の自己PR素材を組むとき、過去の作業ログや Vault の記述から「YD がやったこと」を抽出している。だがログには Claude が実行した作業も同じ粒度で書かれており、両者を区別していなかった。

**過去のやらかし**:
- 2026-09-16: TYO の ES (Q6) と課題PDFに、次の2件を YD 本人の実績として書いた。①プペルのエンドロールを写真と短尺動画だけから4K・3分53秒に組み、279名分のテロップを欠番ゼロで載せ、v17まで作り直して希望納期の8日前に納品 ②「なんか暗くない?」という指摘を基準カメラFX30の実測値±10%に置き換えて解いた。YD の指摘「これただのAIの作業だから思いきり的外れです」で発覚。どちらも Claude がやった作業だった。
- この2件は `knowledge/career/アマナ_志望動機素材.md` にも「納期8日前倒しの実績が未使用」「曖昧な指摘を基準に置き換えた実績が未使用」として、YD の未使用の武器という形で記録されていた。つまり誤りは 2026-09-09 の時点から Vault に入っていて、9日間気づかれずに増幅していた。
- ES は未送信だったため実害は回避。もし出していれば、面接の深掘りで本人が説明できない事態になっていた。就活において最も危険な種類のミス。

**正しい挙動**:
- 自己PR・ES・レジュメの素材にする前に、各エピソードについて「これを実際に手を動かしてやったのは誰か」を1件ずつ YD に確認する。ログに書いてあるだけでは根拠にならない
- 特に技術的な詰め (数値化、自動化、大量処理、バージョンを重ねた作り込み) は Claude がやった可能性が高い。ここは必ず疑う
- YD の実績として確実なのは、Claude が介在していない領域 (Salamat の組織運営、Apple の接客とデモ設計、対面の交渉、Arte Grow の立ち上げ) と、YD が判断・決定した部分
- AI を使って回していること自体は事実であり強みになる。「一人で手作業でやり切った」ではなく「AI を道具にして個人で受注から納品まで回している」と書けば嘘がない

**再発防止**:
- `knowledge/career/` の既存素材を棚卸しし、Claude がやった作業に由来する記述を洗い出す (未実施、要対応)
- 新しい実績を素材ファイルに書くときは、末尾に実行者を明記する (例: 実行=YD / 実行=Claude / 共同)
- ES・自己PR の草案を出すときは「この中で本人が手を動かしていないものはありますか」と必ず一度聞く
- 関連: [[TYO_アピール設計_2026-09-16]] / [[アマナ_志望動機素材]]
