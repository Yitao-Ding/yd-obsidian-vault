...
- **状況**: 🟢 素材ほぼ揃った — 2026-08-22 後日リンク8本受領→DL・演目フォルダ統合済み (1-1, 1-3, 1-7, 2-4, 2-9追加分)。同日夜に1-9再アップ (jpeg3) + 2-5追加分 (jpeg13+mp4 2、計28件) も受領。**全20枠中18枠 (17演目+guest) 受領済み・計161ファイル**
- **欠番 (残1)**: 2-2 さえ のみ — リンク期限切れで取得不可 → **はるきさんに再アップ依頼が必要**。1-6 瀬梨奈は8/23再アップ受領 (動画8本) → 全20枠中19枠受領済み。2-4に音源 `ogawan number.mp3` 同梱 (用途要確認)
...
- **★ Part B素材確定 (2026-08-02 YD指示)**: `/Volumes/Extreme pro/ぷぺるコレオ/メイキング素材/` のメイキング撮影素材 (175クリップ/114GB/4K60p HEVC/BT.709 LUT不要/総135分/撮影6-21)。演目フォルダの縦iPhone動画ではない
- **実装**: Remotion採用・デザイン先行・工程ゲート3段 (スタイルフレーム→1演目5秒→全編)。**制作正本=`ALL WORKs/プペル_エンドロール素材/HANDOVER.md`**
- **★ テロップ表記リスト受領 (2026-08-23)**: 全20枠 (演目名/choreographer/guest/assistant/member 計279名) + スタッフ5枠 (舞台/照明/音響/ぷぺるteam/町長)。正=`_テロップ表記リスト_受領20260823.md`、構造化済=`remotion/src/data/programs.json`。演目順もM番号で確定。細部要確認: 2-7の☺絵文字 / guestみかづきんぱにー詳細 / 2-9のguest,assistant見出し
- **★ 全編サンプルv2ロール版完成 (2026-08-23、YDFB「参考動画みたいに」反映)**: v1の静止画面切替を廃し、参照準拠の**映画式等速ロール** (下→上、カット0) + ポラロイド複数枚フリップ (写真+6秒動画クリップ、計77点、2.4s間隔) + 背景横ドリフト + 煙突3本の煙アニメ + 星瞬き。`remotion/out/endroll_sample_v2_roll.mp4` (原本102MB) + 送付用27MB、YD送付済み。仮=メディア自動セレクト/PartB背景/2-7絵文字。実装=`remotion/src/Endroll.tsx` (決定論組版、v1静止版は`Endroll_static_v1.bak.tsx.txt`)
- **未確定 (残)**: 納品形式(2:02切りか黒含め全長か) / 納期 → 正本=`_案件仕様.md`
