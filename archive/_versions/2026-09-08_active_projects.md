...
### プペル ゆかやねん夜 iPhone素材 (IMG_2202) FX30色合わせ + 音声差し替え (2026-09-08 v5 完了・ギガファイル便アップ中)
- **状況**: 🔄 v5 完成・Extreme Pro 転送済み。ギガファイル便アップ中 (2026-09-08 20:44)。完了後はるきさん確認が次ステップ。v5 は 6 場面すべて FX30 の ±10% 前後に収まり品質 OK。v5 ファイル: `ゆかやねん_夜_IMG2202_FX30match.mp4` (Extreme Pro)。ローカル旧版は `ゆかやねん/_old/`
- **v4→v5 (09-08)**: v4 レンダー後に per-sec コンタクトシートで目視確認 → 暗部の gain 曲線を再調整 (lift.py の smoothstep パラメータ微修正) → v5 レンダー (6892 フレーム、957s)。全6場面 FX30 ±10% 以内を確認して採用
- **v3→v4 (09-08)**: YD 指摘「明るさがまだ足りない」→ v3 の暗い場面 (p99輝度 < 90) を動的に持ち上げる lift.py を追加。フレームごとに darkness スコアを算出し gain を 2.4倍まで smoothstep 補間。render3.py (`/tmp/yuka/`) + params_v12.npy / model12.pt 流用
- **v2→v3 (09-06)**: YD 指摘「暗い場面が暗い」→ グローバル色変換がモヤ場面の中間調を潰していた。フレームごとのモヤ減算 (veil) を追加 (`fit12.py` / `model12.pt` / `params_v12.npy` / `render2.py`)。FX30 と同画角の比較で暗い場面のダンサーが FX30 と同程度に。FX30 自体が暗い演出なので、それ以上は別のルック判断
- **v1→v2**: YD 指摘「ソロスポットのビームが白飛び」→ 原因は FX30 寄りが操作カメラで曲中にパン・ズームしていたのに固定ホモグラフィで処理していたこと。1 秒ごとの追跡 (SIFT + ECC、155/229 秒) に変え、露出比・色変換を学習し直した。mistakes 追記済
- **素材**: `~/Downloads/IMG_2202.mov` (iPhone 12 Pro 引き、夜公演) + 音声・色基準 = `ゆかやねん_夜_寄り.MP4` (FX30)
- **同期**: FX30 が 0.3728s 遅れて開始、ドリフト -25.7ppm。出力と iPhone 元音声の波形相関 +3〜6ms
- **色**: 時間追跡ホモグラフィで同一画素ペアを作り、露出比 (top-20画素) を外してから アフィン+MLP を分布一致で学習。色相と黒の浮きは一致。ビームとダンサーの明るさ比 (iPhone は正面散乱で光線が明るい) は点変換では合わず、白スポット場面はダンサーがやや暗め
- **作業一式**: `プペル/number/_grade/ゆかやねん_IMG2202_色合わせ/` (README + scripts + model9.pt + gain_t_v9.npy + H_track.json)。知見 = [[iphone_to_fx30_color_match]]
