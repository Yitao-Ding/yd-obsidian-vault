...
- **★ ドメイン ✅取得完了 (2026-09-03)**: `hatachitachi.com` / `studiometali.com` をお名前ID 20382623 (団体名義) でクレジットカード決済完了、2件とも「受付完了」。合計1,894円 (各750円+サービス維持調整費197円、Whois情報公開代行0円込み)。**⚠️ ドメイン情報認証メールを2週間以内 (=9/17まで) にクリックしないとドメインが停止する**。RDAPにはまだ反映されていない (レジストリ伝播待ち)。1年登録なので自動更新ONの確認も要
- **★ 位置づけ (2026-09-03)**: ハタチたち / スタジオメタリはどちらも彼女の団体。YDは代行で制作・技術まわりを引き受けているだけで、アカウント名義は団体側にある
- **次のアクション (YD作業)**: ①**ドメイン情報認証メールをクリック (9/17期限、最優先)** ②microCMSアカウント+API3本作成 → `docs/microcms-setup.md` (フィールドID1文字違いで無言でフォールバックに落ちる) ③素材提供 → `docs/assets-list.md` ④確認5件 (LOCK BACK表記 / 「二十歳の台」→「代」/ アーカイブ枠数 / 応募導線の優先度 / 赤の濃さ) ⑤GitHub・Vercel を団体共通アカウントで作成 → `docs/accounts.md` に記入
