...
### 3. 平成たち祭 動画制作
- **★ 2026-09-16 ハタチたち5 説明会 v3 修正ビルド中**: YD から v2 の QC ミス多数を一括指摘: 噛み音声 4 箇所以上使用 / 顔丸窓なし (0:40) / 顔ボケショット (2:00) / 画面 3 往復 (構成員セクション) / 顔位置逆 (6:30) / 字幕単語途中改行 / エンドカード白文字白背景 / パートリーダー BGM 大きすぎ。→ v3 として修正対応中 (FCPXML build_fcpxml_v2.py + wrap_ja.py + QC シート v3)。YD 指示「プロの監査役をつけるようにして」= 別エージェントによる QC パスを最終工程に追加する方針。
