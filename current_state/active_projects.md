---
type: current_state
last_updated: 2026-09-16 03:15 (ハタチたち5 説明会 構成v6_v3 完成・FCP読み込み確認済・YD判断待ち)
...
- **✅ PD 指摘2件 対応 (2026-09-05 15:55 LINE、push済み・本番反映確認済み 17:02)**: ①スマホのヒーローで「ハタチ」「たち」を写真の縁に寄せる → デスクトップと同じ「写真が字の2割を食う」構図に (top 25% / bottom 27.9%、左右も写真に揃え left 16% / 51%)。375/430px 実測で重なり 23%、縦書きとの間隔 26px+。A (重ねる) を出したら YD「食い込んでる」→ B (重ねない、字面と写真の隙間 約10px、top 20.4% / bottom 24.6%) に変更 commit `146c334`、push済み・本番反映確認 17:58。スマホでは重ねない・デスクトップは重ねる、で確定 ②LOOK BACK のハタチたち5・平成たち祭を Instagram へリンク → `ArchiveItem.linkUrl` 追加 (videoUrl より優先)、制作中枠に Instagram の印 (SVG、文字は珊瑚上でコントラスト不足)。レビューWF (3観点→反証、13エージェント) で確定1件 = docs/microcms-setup.md の linkUrl 未記載 → 追記済み。反証で落ちた「ハタチ左端が写真より3%外」は PD の丸印とも合うので採用して揃えた
- **✅ セキュリティ整備 (2026-09-05 実装、2026-09-06 01:03 push・本番反映確認済み)**: 監査WF 4観点→反証 (24エージェント、13完了/11がセッション上限で未検証)。実装: セキュリティヘッダ7本 (CSP は enforce 3 directive + 通信先 Report-Only、理由は [[2026-09-05_ハタチたちHP_セキュリティ整備]]) / `*.vercel.app` → 独自ドメイン 308 / preview ルート 404化 / x-powered-by 停止 / next 16.3.4 + react 19.2.8 (`pnpm audit` 14→0) / pnpm minimumReleaseAge 7日 / dependabot.yml / `.serena` を gitignore / `docs/security.md` 新規。設定: GitHub Dependabot alerts+security updates ON、ruleset (main の削除・force push 禁止)、Vercel `NEXT_PUBLIC_SITE_URL` Production 設定。検証: 本番ビルドで curl 7ヘッダ、Playwright 1200/375 で Instagram 2枚・YouTube 再生・Console に CSP 違反0。**YD 手作業 (優先順)**: ①お名前.com ドメイン情報認証メール 9/17 期限 ②お名前 Navi 移管ロック+2FA ③GitHub/Vercel/Gmail 2FA ④Vercel Hobby 非商用条項の判定 ⑤UptimeRobot (任意) ⑥素材原本の恒久保管。詳細 = repo `docs/security.md`
- **残 (Claude作業)**: ~~`NEXT_PUBLIC_SITE_URL` 設定して robots.txt の検索避け解除~~ → 2026-09-05 確認: 独自ドメイン割当後は Vercel の `VERCEL_PROJECT_PRODUCTION_URL` が hatachitachi.com になり、robots.txt は既に Allow + sitemap 出力済 (env 設定不要)。残は Phase3 実素材差し替え + セキュリティ整備 (2026-09-05 着手)
- **★ セキュリティ監査 部分完了 (2026-09-05)**: dependencies 観点のみ完結。next@16.2.6 に pnpm audit 14件 + critical 2件 (GHSA-2xp9-vwfh-vxw4 AVIF RCE / GHSA-p293-qw3h-jr36 Windows RCE)。**実害リスクは低** (Server Actions / middleware / rewrites なし、`/_next/image` は Vercel edge が処理)。**推奨: `next@16.3.4` へバンプ** (コマンド = [[2026-09-05_ハタチたちHP_next16.3.4セキュリティ監査結果]])。repository hygiene 等 7観点は未実行 (セッション上限タイムアウト)
...
- **次のアクション**: ①~~LINE送付~~ ✅納品済み (2026-08-11 YD確認) ②FB次第でv2 → **⚠️ 注意 (2026-09-09 訂正): 納品mp4は消失していない。`~/ALL WORKs/ぷぺる_2026-08/納品/プペルOP映像_曲フル尺_v1.mp4` (32.8MB) と確認用_頭36秒_720p.mp4 が内蔵に実在することを実走査で確認。消失しているのは制作パイプラインと素材15本のローカル分のみ (2026-08-11棚卸し)。v2再レンダーが必要になったらギガファイル素材再DL+再構築が前提** ③請求 (言い値+曲購入費実費) **← ⚠️ 監査指摘 2026-08-25: 納品から2週間経過で未対応。最優先対応**
...
### プペル ゆかやねん夜 iPhone素材 (IMG_2202) FX30色合わせ + 音声差し替え (2026-09-08 v5 完了)
- **状況**: ✅ v5 書き出し・アップ完了 → `プペル/number/ゆかやねん/ゆかやねん_夜_IMG2202_FX30match.mp4` (3840×2160 / 30p / HEVC 10bit 27.0Mbps / 229.7s / 777MB / AAC 320k)。ギガファイル便 **https://23.gigafile.nu/1217-o2a6e2fd605816e7dcd11f9597311be89** (期限 2026-12-17)。はるきさんへの送付は YD。v1〜v4 のリンクは使わない。ローカル旧版は `ゆかやねん/_old/`
- **v3→v5 (09-08)**: YD 指摘「96〜100 秒の群舞と 110 秒のトップスポットがまだ暗い。目的は FX30 に合わせること」。実測で v3 は暗い場面の人物が FX30 より 0.6〜1.1 段暗かった (v3 の「同程度」は誤り)。暗さに応じてリニアで中間調を持ち上げる曲線 (最大 2.4 倍、白を固定するのでビームは飽和しない、明るい場面と暗転は無変更) を後段に追加 (`render3.py` + `darkness_t.npy`)。v4 を全区間コンタクトシートで確認し、3 区間を手で修正して v5 (ソロスポット 51〜58 秒は上限 1.4 / 白スポット 166〜181 秒は上限 1.15 = v3 で既に同等だった / 159〜165 秒は 3.0 に増量)。6 場面で FX30 の ±10% 前後、音声同期 +6ms。秒ごとの自動補正案は視差で値が暴れて不採用
- **v2→v3 (09-06)**: モヤ減算 (veil) 追加。**v1→v2 (09-05)**: FX30 寄りのパン・ズームを 1 秒ごとに追跡して白飛び修正
- **素材**: `~/Downloads/IMG_2202.mov` (iPhone 12 Pro 引き、夜公演) + 音声・色基準 = `ゆかやねん_夜_寄り.MP4` (FX30)。`ゆかやねん_夜_number.mov` は別公演 (緑照明) で基準にならない
- **同期**: FX30 が 0.3728s 遅れて開始、ドリフト -25.7ppm
- **作業一式**: `プペル/number/_grade/ゆかやねん_IMG2202_色合わせ/` (README に v5 まで記載)。/tmp/yuka は再起動で消えるので再現時は README の手順で作り直す。知見 = [[iphone_to_fx30_color_match]]
...
### 3. 平成たち祭 動画制作
- **★ 2026-09-16 深夜 ハタチたち5 説明会 構成v6_v3 完成 (FCP 読み込み・実画確認済、YD 判断待ち)**:
  - 納品 `_FCP納品/ハタチたち5説明会_構成v6_v3.fcpxml`。4K 59.94p、本編 6:22.2 (382.2秒) + エンドカード6秒、ショット49・字幕34枚・丸窓28クリップ。DTD 検証 OK、構造チェック0件。YouTube 用全文 SRT 95件 (`_FCP納品/字幕_構成v6タイムライン基準_全文.srt`、生成器 `_cc_work/make_srt_v6.py`)
  - v3 の残存不良を機械検査で洗って v4→v5→v6 で全部潰した: 撮影場所の言い直し採用 (src 226-232)、次の文の言いかけ2件、カット境界の単語断片15か所、丸窓トラックの偽陽性 (スクリーンの文字・集合写真を顔として切り抜き、v3 valid の17%)、丸窓の点滅、スライドズームのカットごとリセット、ハードカットのクリック音、p7 で丸窓が「その他関係者」を隠す、E p8 の話者見切れ、p15 タイルの写り込み、p17 クイズ動画の横切り、p20/p21 の言い直しと言い間違い
  - 監査体制: 別エージェントで4観点 (噛み/映像QC/字幕/FCPXML構造) 洗い出し → 各指摘を別エージェントが反証 (round1 51件、round2 22件) → 私が判定 → 修正、を2周。YD指示で Fable は判断のみ、洗い出し・反証・実装・FCP操作は Opus/Sonnet
  - v5_v2 は FCP 読み込み確認済 (メディア欠落ゼロ、丸窓実寸OK、字幕16点全件)。v6_v2 は FCP 読み込み・12点実画確認済 (丸窓修正4本OK・p7小円OK)。v6_v3 (p10 intro を p9 に、p8 後半をワイドに戻す) も読み込み・3点確認済 (参照64件欠落ゼロ)。YD 判断待ち: SV の丸窓なし / 話者フレーム外の約100秒 / p8 丸窓が「20:20公開」行を隠す / p8 後半の投影ワイド (白飛び) vs スライド / Canva 原本 / p14 動画版 / 座談会指示 / 旧イベント削除 / assets 整理
  - **YD 判断待ち**: ①SV (p10-13) の丸窓なし (名前ラベルと重なるため、YD未確認の判断) ②話者がフレーム右外の約100秒は丸窓なし ③Canva 内動画の原本 (1080p 止まりで V/SV が4K拡大) ④p14 の動画入り版 (今はワイド撮影映像で代用) ⑤座談会の指示 ⑥旧イベント/旧 fcpxml/assets 44GB の整理
  - 引き継ぎ = プロジェクト直下 `HANDOVER.md`、説明 = `_FCP納品/README.md`。知見 [[fcpxml_auto_edit_pipeline]]、決定 [[2026-09-16_ハタチたち5説明会_納品前監査体制]]
- **★ 2026-09-15 ハタチたち5 説明会 構成v2 完成 (FCP読み込み済み)**: 説明会パート 7:17 + エンドカード 6秒、4K 59.94p。YD指示を全反映: 字幕はスライド外のみ (撮影映像+動画の上、56本)、カット頭0.35秒・尻0.55秒の間、RNNoise整音 + -16LUFS、黄かぶり動画をグレーワールド補正 (p17クイズのみ補正不能で元のまま)。p9/p15-19は動画全画面。Canva書き出し1080p止まりのため全画面・スライド+動画パートは4K拡大 (Canva元動画をもらえれば差替可能)。納品フォルダ: `_FCP納品/` (fcpxml・全文SRT・整音聴き比べm4a・座談会_話題マップ.md・README・assets)。**座談会C008話題マップ完成** (33分2部構成、16話題、recruit_value上位4区間特定済み、`_FCP納品/座談会_話題マップ.md`)。**YD待ち3点**: ①座談会パートの指示 (尺配分/テロップ/カット方針) ②Canva内動画の原本 ③p14の動画入り版。**名称統一済み**: 「ハタチたち」に全統一 (座談会文字起こし32箇所: 私たち/20歳たち/二十字たち/ハダチたち → ハタチたち。説明会字幕は元から統一済み)。1:44〜2:15の運営4〜5人目自己紹介と2:40前後の3期参加者名は文字起こし崩れのため名前テロップは映像確認要。**座談会顔検出+口の動き解析実行中** (8人、33分、`_cc_work/zadankai_faces.py`、バックグラウンド実行中)。手順知見 [[fcpxml_auto_edit_pipeline]]、決定 [[2026-09-15_ハタチたち5説明会動画_FCP自動編集]] / [[2026-09-15_ハタチたち説明会座談会_合体動画構成決定]]
- **★ 2026-09-15 ハタチたち説明会 FCPXML v1 生成済み・FCP インポート待ち**: FCPXML `_FCP納品/ハタチたち5説明会_edit_v1.fcpxml` 生成・DTD 検証済 (18:20)。QC シート確認済。Canva スライド内動画 11 本切り出し完了 (canva/video/p*_full.mov)。字幕 JSON 完成。スクリーン四隅キャリブレーション済 (screen_quad_4k.npy)。FCP ライブラリ選択ダイアログ操作待ち (18:25 時点)。[[2026-09-15_ハタチたち説明会_編集スタイル決定]]
- **★ 2026-09-15 vol.2 (9/14公演) 定点15本 ギガファイル納品完了**: `~/Downloads/平成たち祭定点/` にナンバー名リネーム済み (香盤順 01〜15)、1本1リンク・期限 2026-12-24、一覧 `納品リンク一覧.txt`。手順は [[gigafile_cli_delivery]]
- **★ 2026-09-14 YouTubeサムネイル B案確定 (Claude Design)**: 平成文化祭風 (珊瑚#ff5757+紺#1f469d、罫線紙にプリント写真)。写真=ハイミー Hi,Me:) の出演カット (黒ブレザー+ネクタイの2人組、Canon `0U4A2711.MP4` 69.5秒、両手を上げて笑う瞬間)。キャンバス= https://claude.ai/code/artifact/530607e4-68ac-413f-8bdd-57236cfea345 (page-1 採用B / page-2 没A・C)。C-Log3→709 は自作LUT (CinemaGamut→709、彩度72%)。音先mp3は旧Macロストで現存せず、演目特定は衣装(コナン風)で行った **→ 2026-09-14 11:40 YD「どっちも書き出し」で2版納品**: 黒ブレザー2人組 (`0U4A2711` 69.5s) / 青ブレザー+赤蝶ネクタイ+眼鏡 (`0U4A2713` 45.0s、金髪の方。同衣装の黒髪眼鏡は司会)。PNG は scratch 直下 `平成たち祭_サムネ_黒ブレザー.png` / `_青ブレザー.png` (セッション消滅で消えるので必要なら Extreme pro へ移すこと) **→ 12:07 ギガファイル便 100日で納品**: https://19.gigafile.nu/1223-083cd05263ed767cac751bc16f87d254 (mp4 `04_2min_v3_chrono.mp4` 896MB + サムネ青/黒、期限 2026-12-23、削除キー a18d)
...
- **★ 2026-09-14 YouTubeサムネイル3案作成**: 素材226本を全数目視 (Canon76/Sony117、Log→709: C-Log3は自作 clog3_709_v2.cube / S-Log3はResolve付属LUT) → 候補3本 (0U4A2708/2706/A016C056) を1秒刻み精査 → Claude Designキャンバスで1280×720サムネ3案。**B案 (平成文化祭風: 珊瑚#ff5757+紺#1f469d・学級新聞風) をYD採用決定**。写真はハイミー (コナン衣装=ブレザー+赤リボン+眼鏡) の演目カットを使用予定、対象クリップ特定作業を継続中 (0U4A2688〜周辺を精査中)
...
### 9. アマナ 1day採用 一次選考
- **状況**: 自己PR原稿・スライド構成確定。面接 2026-09-10
- **形式**: 自己PR5分、スライド4枚。課題「求む、妥協なきこだわり」
- **次のアクション**:
  - [ ] 対面/オンライン確認・投影環境確認
  - [ ] 見せる作品の最終選定
  - [ ] 「2倍」の根拠確認 (部隊数か渡航希望者数か)
- **関連**: `knowledge/career/アマナ_志望動機素材.md`
