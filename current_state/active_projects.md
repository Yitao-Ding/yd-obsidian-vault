...
- **★ v12レンダー中 (2026-08-28)**: Part B swap2 — ①Osmo(Log)カット(swap7) → A021C043_260621UJ 17.6-20.8 (撮影中のモニターチェック寄り、タブレットに写真が映り手がスワイプ) ②ラストカット(swap10) → A021C177_260621LA 1690.8-1694.3 (私服に着替えた7人が全員カメラに手を振る撤収シーン)。+フェードイン+前半→後半1.5秒フェード追加。swap10は他より一段暗く緑に転んでいるため補正当て済み (brightness+0.07/contrast1.2/sat1.15)。`cuts.json`は`cuts_v2_10cut.json`にバックアップ済み
- **未確定 (残)**: ~~Part B メイキングrecut~~ ✅ / 2-9のguest,assistant見出し / 納品形式(2:02切りか黒含め全長か) / 納期 → 正本=`_案件仕様.md`
