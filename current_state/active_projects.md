...
- **状況**: 音声同期完了・カラーグレード 3D LUT 生成中
- **素材**: `琴_夜_number.mov` (FX30 24/30fps VFR) + `琴_夜_寄り.MP4` (iPhone 寄り) @ `/Volumes/Extreme pro/プペル/number/琴/`
- **音声オフセット確定**: FX30 が MP4 より **11.41 秒先行** (相互相関残差 4ms)。色合わせ手順.html 実測 11.4s と一致
- **クロックドリフト**: -97.7 ppm (全長 -26 ms) → 補正要
- **カラーグレード**: Curve A で青 9.6%・赤 2.1% 飽和 → ソフトクリップ (フィルミックロールオフ) 組み込みの 3D LUT を生成中
- **次のアクション**: 3D LUT 完成 → ffmpeg で Mov に適用 → 音声を MP4 音声に差し替えて最終出力
- **知見**: [[multicam_audio_sync_crosscorrelation]]
