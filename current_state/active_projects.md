---
type: current_state
last_updated: 2026-09-16 00:16 (ハタチたち5 説明会 v3 修正ビルド中)
...
### 3. 平成たち祭 動画制作
- **★ 2026-09-16 01:03 ハタチたち5 説明会 v4 生成完了・4エージェント監査中**: `_FCP納品/ハタチたち5説明会_構成v4_v1.fcpxml` DTD検証済み。v3→v4 修正内容: 丸窓トラックをSFaceで全フレーム本人判定して再構築 (有効17%→精度向上)、カット境界をverbatim単語タイムスタンプ+短窓whisper聞き直しで詰め (断片15か所、噛み1件、言いかけ2件解消)、同一スライド内ズームを連続化 (カットごとリセット廃止)、字幕折り返しをfugashi形態素解析で強化。4エージェント監査 (噛み/映像QC/字幕/FCPXML構造) 実行中 — 3名反証通過後に確定指摘を v4_v2 反映。**YD承認**: 「それ終わったらfcpいじっていいよ」= 監査完了後にFCPインポート可。知見更新済み [[fcpxml_auto_edit_pipeline]]
