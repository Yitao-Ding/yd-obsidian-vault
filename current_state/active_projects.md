...
- **残 (Claude作業)**: ~~`NEXT_PUBLIC_SITE_URL` 設定して robots.txt の検索避け解除~~ → 2026-09-05 確認: 独自ドメイン割当後は Vercel の `VERCEL_PROJECT_PRODUCTION_URL` が hatachitachi.com になり、robots.txt は既に Allow + sitemap 出力済 (env 設定不要)。残は Phase3 実素材差し替え + セキュリティ整備 (2026-09-05 着手、監査WF実行中)
