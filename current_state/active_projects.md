...
- **状況**: 🟢 **素材セレクト・再配置 進行中 (2026-08-28)** — 全20枠カード素材を「写真先頭・類似写真は1枚・動画はベストシーン切り出し」ルールで再セレクト中。workflow wf_ec0dee5d-e6d 実行中 (11起動/3完了 as of 10:08)。**カード表示仕様判明: 1枠あたり約10.2秒・1点2.4秒 → 実質4点まで** (5点目以降は画面外に抜けるため誰も見ない)。Endroll.tsx修正: 動画カードの再生タイミング (`startFrom`) + 縦素材の顔切れ対応 (`objectPosition` フィールド追加)。**YD確定: 動画しかない枠 (1-6瀬梨奈・guestみかづきんぱにー) は動画始まりでOK**。素材全数受領完了 (2026-08-26) + 全編v7レンダー済み (2026-08-27)。2-2さえ再アップ受領 (jpeg1+mov1) で**全20枠 (19演目+guest) 欠番ゼロ**。`remotion/out/endroll_sample_v7.mp4` (1080p 124s) + 送付用30MB = 2-2込み最新版。2-4に音源 `ogawan number.mp3` 同梱 (用途要確認)
