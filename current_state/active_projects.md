...
- **✅ PD 指摘2件 対応 (2026-09-05 15:55 LINE、push済み・本番反映確認済み 17:02)**: ①スマホのヒーローで「ハタチ」「たち」を写真の縁に寄せる → デスクトップと同じ「写真が字の2割を食う」構図に (top 25% / bottom 27.9%、左右も写真に揃え left 16% / 51%)。375/430px 実測で重なり 23%、縦書きとの間隔 26px+。重ねない案 (B) も検討したがデスクトップとの整合で A を採用、YD 確認待ち ②LOOK BACK のハタチたち5・平成たち祭を Instagram へリンク → `ArchiveItem.linkUrl` 追加 (videoUrl より優先)、制作中枠に Instagram の印 (SVG、文字は珊瑚上でコントラスト不足)。レビューWF (3観点→反証、13エージェント) で確定1件 = docs/microcms-setup.md の linkUrl 未記載 → 追記済み。反証で落ちた「ハタチ左端が写真より3%外」は PD の丸印とも合うので採用して揃えた
