...
### 3. 平成たち祭 動画制作
- **★ 2026-09-16 00:47 ハタチたち5 説明会 v4 準備中**: v3 QC 継続。YD「FCP は触らないで」指示でバックグラウンド解析のみ実施。`cuts_v4.json` 生成 (p2/p4a/p4b/p5a/p5b タイムコード修正)、`wrap_ja2.py` 作成 (fugashi 形態素解析による字幕折り返し改善)、PIP face validation (`pip_validate2.py`: YuNet + SFace 類似度スコア算出)、カット境界音声チェック (`edge_listen2.py`: mlx_whisper で噛み/言いかけ検出)、無音区間検出 (>=2.4s ランを特定)。→ FCP インポートは YD の PC 作業終了後。
