...
- **★ v13確認・v15完成・v16/v17音源差し替え (2026-08-28)**: v13で2カット確認済 (楽屋鏡前支度・白ホリ紙吹雪は指定通り)。v14=1-2ミス写真除去。v15=IMG_0169(自撮り集合)組み込み。v16=音源を えんとつまちのプペル の2:01から差し替え (元ファイル保持=`pupelle_full_original.m4a`)。v17=ラッパアタックが元ファイル121.65sと判明→頭0〜0.55sを無音にして0.65sでラッパをフルイン・映像を124s→123sに詰め (メイキング32sはそのまま)。v17レンダー中 (23:34時点)
...
- **未確定 (残)**: 2-9のguest,assistant見出し / 納期 / ~~納品形式~~ → 音源が2:01-4:04(123秒)確定で映像尺も123秒に整合。4K本レンダー未実施 → 正本=`_案件仕様.md`
