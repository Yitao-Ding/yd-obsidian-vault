...
published_at: '2026-07-23T15:42:40+00:00'
collected_at: '2026-07-23T19:06:13.675728+00:00'
score: 10.0
importance: 4
categories:
- agents
...
OneCLI は、AI エージェントと外部サービスの間に位置するネットワークゲートウェイです。エージェントに認証情報を直接渡すのではなく、リクエスト時に認証情報をインジェクトして安全に管理します。Rust で書かれたプロキシサーバーとして動作し、AES-256-GCM で暗号化した状態で秘密鍵を保管します。任意のエージェントフレームワーク (Claude、Cursor、OpenClaw、MCP など) に対応し、Docker で簡単にデプロイできます。機密操作には人間による承認プロセスを追加でき、詳細な監査ログで安全にエージェントを統制します。
...
*Generated by ai-researcher at 2026-07-23T19:06:13.676475+00:00*